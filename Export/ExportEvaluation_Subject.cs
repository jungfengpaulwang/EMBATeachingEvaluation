...
{
    public partial class ExportEvaluation_Subject : BaseForm
    {
        ...
        public ExportEvaluation_Subject()
        {
            InitializeComponent();
            this.Text = "列印課程評鑑統計";
            this.TitleText = "列印課程評鑑統計";
            Query = new QueryHelper();
            Access = new AccessHelper();

            this.Load += new EventHandler(ExportEvaluation_Subject_Load);
        }

        private void ExportEvaluation_Subject_Load(object sender, EventArgs e)
        {
        ...
        //    Dictionary<string, Dictionary<string, List<DataSet>>> file_TeacherStatistics = new Dictionary<string, Dictionary<string, List<DataSet>>>();
        //    Dictionary<string, Workbook> dicFiles = new Dictionary<string, Workbook>();
        //    Dictionary<string, Dictionary<ReportHelper.CellObject, ReportHelper.CellStyle>> dicStyles = new Dictionary<string, Dictionary<CellObject, CellStyle>>();
        ...
        //            if (!dicStyles.ContainsKey(SchoolYear + "-" + Semester + "-" + SubjectName))
        //                dicStyles.Add(SchoolYear + "-" + Semester + "-" + SubjectName, new Dictionary<CellObject, CellStyle>());

        //            Dictionary<ReportHelper.CellObject, ReportHelper.CellStyle> dicCellStyles = dicStyles[SchoolYear + "-" + Semester + "-" + SubjectName];

        //            string key = CourseName + "-" + TeacherName;

        //            if (!dicCourses.ContainsKey(SchoolYear + "-" + Semester + "-" + SubjectName))
        //            {
        ...
        //                dicCourses.Add(SchoolYear + "-" + Semester + "-" + SubjectName, o);
        //            }

        //            if (!dicTeachers.ContainsKey(TeacherID))
        //                dicTeachers.Add(TeacherID, TeacherName);
        ...
        //            //string key = xStatistics.Attribute("TeacherName").Value;
        ...
        //            if (!file_TeacherStatistics.ContainsKey(SchoolYear + "-" + Semester + "-" + SubjectName))
        //                file_TeacherStatistics.Add(SchoolYear + "-" + Semester + "-" + SubjectName, new Dictionary<string, List<DataSet>>());

        //            if (!file_TeacherStatistics[SchoolYear + "-" + Semester + "-" + SubjectName].ContainsKey(key))
        //                file_TeacherStatistics[SchoolYear + "-" + Semester + "-" + SubjectName].Add(key, new List<DataSet>());

        //            file_TeacherStatistics[SchoolYear + "-" + Semester + "-" + SubjectName][key].Add(dataSet_PageHeader);
        //            file_TeacherStatistics[SchoolYear + "-" + Semester + "-" + SubjectName][key].Add(dataSet_DataSection);

        //        }
        //        if (file_TeacherStatistics.Count == 0)
        //            throw new Exception("沒有資料。");

        //        foreach (string key in file_TeacherStatistics.Keys)
        //        {
        //            MemoryStream ms = new MemoryStream(Properties.Resources.TeachingEvaluationTemplate);

        //            Dictionary<string, List<DataSet>> dicTeacherStatistics = file_TeacherStatistics[key];
        //            //Dictionary<string, List<DataSet>> dicSingleTeacherStatistics = new Dictionary<string, List<DataSet>>();
        //            //foreach (string key_teacher in dicTeacherStatistics.Keys)
        //            //    dicSingleTeacherStatistics.Add(dicTeachers[key_teacher], dicTeacherStatistics[key_teacher]);

        //            Workbook wb = Report.Produce(dicTeacherStatistics, ms, false, dicStyles[key]);
        ...
        //            dynamic o = dicCourses[key];

        //            string file_name = key + "-課程教學評鑑統計表";
        //            dicFiles.Add(file_name, wb);
        //        }
        //    });
        ...
        //            try
        //            {
        //                dicFiles[fileName].Save(Path.Combine(filePath, fileName + ".xls"), FileFormatType.Excel2003);
